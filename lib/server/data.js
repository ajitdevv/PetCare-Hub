...
// Ensures auth.users + profiles entries exist for a user before FK-dependent writes.
// Called lazily so existing users (registered without auth sync) are fixed on first write.
async function syncUserProfile(currentUser) {
  if (!currentUser || currentUser.id === ADMIN_USER.id) return;

  // Try to create auth user with the SAME UUID — lets us control the id
  // so profiles.id matches the user's public.users id.
  await sb().auth.admin.createUser({
    id: currentUser.id,
    email: currentUser.email,
    password: crypto.randomUUID(),         // random — app always uses custom auth
    user_metadata: { name: currentUser.name },
    email_confirm: true
  });
  // Ignore error: user may already exist in auth.users — that's fine.

  // Upsert profiles so the FK chain (table.user_id → profiles.id → auth.users.id) is satisfied.
  await sb()
    .from("profiles")
    .upsert({
      id: currentUser.id,
    ...
    .select();
  // Ignore error: if profile already exists this is a no-op.
}
