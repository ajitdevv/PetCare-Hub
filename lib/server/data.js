...
function mapComplaint(row) {
  if (!row) return null;
  // DB uses open/closed; UI uses Pending/Resolved
  const STATUS = { open: "Pending", closed: "Resolved" };
  return {
    id: row.id,
    _id: row.id,
    userId: row.user_id,
...
export async function registerUser({ name, email, password }) {
  ...
  const userRow = sbCheck(
    await sb()
      .from("users")
      .insert({ name, email: normalizedEmail, password: hashedPassword, role: "user" })
      .select()
      .single()
  );
